...
# Dependency on ${BUILD_LOCATION_ABSOLUTE}/db/docker.built deliberately omitted as a performance measure.
# In principle this is bad because it means that a brand-new, fresh setup of the development environment
# will have failing "make test" initial behavior, since spt-db image will not be around.
# The in-development time savings is high however.
