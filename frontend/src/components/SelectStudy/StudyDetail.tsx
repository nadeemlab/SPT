...
function SampleCohorts({ cohorts }: { cohorts: Cohorts[] }) {
...
function extractSentenceRoute(finding: string) {
  const regex = new RegExp("\. (/.*)?$");
  const match = finding.match(regex)
  if (match == null) {
    return {
      sentence: finding,
      route: undefined
  ...
  return {
    sentence: finding.replace(regex, "."),
    route: match[1]
  }
}

function Findings({ findings }: { findings: string[] }) {
  const sentence_urls = findings.map((finding) => extractSentenceRoute(finding));
  return (
    <StudyItem label="Highlights">
      <table className="text-[13px] leading-4">
        <tbody className="flex flex-col gap-2">
          {sentence_urls.map((item) => {
            const hasUrl = typeof item.route !== "undefined"
            return (
...
export default function StudyDetail() {
  ...
  const findings = useStudy((state) => state.studyData.findings);
  const deleteSelectedStudy = useStudy((state) => state.deleteSelectedStudy);

  if (!studyData.summary) return <Spinner />;
  return (
    <>
      <div className="mx-4 rounded-3xl text-[#2A2B56] bg-white border border-gray-300">
        <h3 className="bg-primary-yellow w-full h-12 flex justify-center items-center rounded-t-3xl p-[20px]">
          {displayStudyName(getStudy()!)}
        </h3>
        <div className="text-sm">
          ...
          <Divider />
          <StudyItem label="Number of channels measured">
      ...
      <div className="flex gap-36 my-4 justify-center">
        <ButtonIcon
          icon={Icons.reset}
          onClick={() => {
            deleteSelectedStudy();
        ...
        />
        <ButtonIcon
          to={`/study/${normalizeStudyName(studyName)}/analysis`}
          icon={Icons.rightx2}
          text="Next"
        />
      </div>
