...
class SchemaInfuser(DatabaseConnectionMaker):
    ...
    def setup_schema(self, force=False):
        ...
        if force is True:
            self.verbose_sql_execute(
                'drop_views.sql', description='drop views of main schema')
            self.verbose_sql_execute(
                None, description='drop tables from main schema',
                contents=self.create_drop_tables())

        self.verbose_sql_execute(
            'schema.sql', description='create tables from main schema',
            source_package='adiscstudies')
        self.verbose_sql_execute(
            'performance_tweaks.sql', description='tweak main schema')
        self.verbose_sql_execute(
            'create_views.sql', description='create views of main schema')
        self.verbose_sql_execute(
            'grant_on_tables.sql', description='grant appropriate access to users')
    ...
    def refresh_views(self):
        self.verbose_sql_execute(
            'refresh_views.sql', description='refresh views of main schema', silent=True)

    def recreate_views(self):
        self.verbose_sql_execute(
            'drop_views.sql', description='drop views of main schema')
        self.verbose_sql_execute(
            'create_views.sql', description='create views of main schema', itemize=True)
        self.verbose_sql_execute(
            'grant_on_tables.sql', description='grant appropriate access to users')

    def verbose_sql_execute(self, filename,
                            source_package='spatialprofilingtoolbox.db.data_model',
                            **kwargs):
        verbose_sql_execute(filename, self.get_connection(),
