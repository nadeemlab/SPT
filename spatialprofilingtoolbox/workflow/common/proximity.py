...
def compute_proximity_metric_for_signature_pair(
    ...
    mask1 = cells[signature1.positive_markers].all(axis=1) & \
        (~cells[signature1.negative_markers]).all(axis=1)
    mask2 = cells[signature1.positive_markers].all(axis=1) & \
        (~cells[signature2.negative_markers]).all(axis=1)
