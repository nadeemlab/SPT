...
from .integrator import FrequencyAnalysisIntegrator
...
class FrequencyAnalyzer(SingleJobAnalyzer):
    def __init__(self,
        ...
        **kwargs,
        ...
        """
        super(FrequencyAnalyzer, self).__init__(job_index=job_index, **kwargs)
        self.dataset_design = dataset_design
        self.computational_design = FrequencyDesign(
            dataset_design = self.dataset_design,
            complex_phenotypes_file = complex_phenotypes_file,
        )
        sample_identifiers_by_file = self.retrieve_cell_input_file_info()
    ...
    def retrieve_cell_input_file_info(self):
        """
        :return: Information about the input fulls with cell data. A dictionary whose
        ...
        for row in result:
            ...
            if sha256 != expected_sha256:
                logger.error('File "%s" has wrong SHA256 hash (%s ; expected %s).', row[0], sha256, expected_sha256)
