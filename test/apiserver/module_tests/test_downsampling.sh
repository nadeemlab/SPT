...
status=$?
if [[ "$status" != "0" ]]; then echo 'Something wrong with metadata.json.'; cat metadata.json; rm metadata.json; fi;
consider_exit $status

diff module_tests/expected_rows.txt rows.txt
status=$?
if [[ "$status" != "0" ]]; then echo 'Something wrong with rows.txt.'; cat rows.txt; rm rows.txt; fi;
consider_exit $status
